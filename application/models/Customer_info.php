...
class Customer_info extends CI_Model
{
	...
	public function create_temp_table(array $inputs)
	{
		...
		if(empty($inputs['sale_id']))
		{
			...
			{
				$where = 'WHERE customer_p.customer_name like ' . $this->db->escape($inputs['customer_name']);
			}else {
				$where = 'WHERE customer_p.phone_number like ' . $this->db->escape($inputs['phone_number']);
			}
		}
		else
		{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_items_temp') . 
			' (INDEX(sale_date), INDEX(sale_id), INDEX(customer_name),INDEX(phone_number))
			(
				SELECT
