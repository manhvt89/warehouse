		...
		else
		{
			...
			for($i = 1; $i < count($sheet_data); $i++) {
				...
				$item_data = array(
					...
					'cost_price'			=> is_numeric(str_replace(',','',$data[4])) == true ? str_replace(',','',$data[4]):0,
					'unit_price'			=> is_numeric(str_replace(',','',$data[5])) == true ? str_replace(',','',$data[5]):0,
					'reorder_level'			=> is_numeric(str_replace(',','',$data[10])) == true ? str_replace(',','',$data[10]):0,
					'supplier_id'			=> $this->Supplier->exists($data[3]) ? $data[3] : NULL,
