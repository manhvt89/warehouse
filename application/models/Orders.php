...
class Orders extends CI_Model
{
	...
	public function get_the_order($sale_id)
	{
		$this->db->select('sales.*,
							payments.payment_type AS shipping_payment_type,
							payments.payment_id  AS payment_id,
							payments.payment_type AS payment_type
		');
		$this->db->from('sales AS sales');
		$this->db->join('sales_payments AS payments', 'sales.sale_id = payments.sale_id', 'left outer');
	...
	public function get_info($sale_id)
	{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_payments_temp') .
			'(
				SELECT payments.sale_id AS sale_id, 
					payments.payment_amount AS sale_payment_amount,
					CONCAT(payments.payment_type, " ", payments.payment_amount) AS payment_type,
					payments.payment_type AS shipping_payment_type,
					payments.payment_id as payment_id
		...
		$this->db->select('
				...
				sales.code as code,
				sales.shipping_address as shipping_address,
				sales.shipping_city  as shipping_city,
				sales.shipping_method as shipping_method,
				sales.shipping_phone  as shipping_phone,
				sales.source as source,
				sales.shipping_address_type as shipping_address_type,
				sales.completed as completed,
				sales.kind as kind,
	...
	public function search($search, $filters, $rows = 0, $limit_from = 0, $sort = 'sale_date', $order = 'desc')
	{
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_payments_temp') . 
			...
			(
				...
				AND sales.kind = 1 GROUP BY sale_id
			)'
		);
		...
		$this->db->query('CREATE TEMPORARY TABLE IF NOT EXISTS ' . $this->db->dbprefix('sales_items_taxes_temp') . 
			...
			(
				...
				INNER JOIN ' . $this->db->dbprefix('sales_items') . ' AS sales_items
					ON sales_items.sale_id = sales_items_taxes.sale_id AND sales_items.line = sales_items_taxes.line
				WHERE DATE(sales.sale_time) BETWEEN ' . $this->db->escape($filters['start_date']) . ' AND ' . $this->db->escape($filters['end_date']) . ' AND sales.kind = 1
		...
		$this->db->select('
				...
				CONCAT(sales.shipping_address, ", ", sales.shipping_city) AS shipping_address,
				sales.shipping_phone,
				sales.shipping_method,
				sales.source AS shipping_source,
				sales.completed,
				sales.shipping_code,
				customer_p.facebook,
	...
	*/
	...
	public function update($sale_id, $sale_data, $payments,$payment_id)
	{
		//$this->db->where('sale_id', $sale_id);
	...
	public function save($items, $customer_id, $employee_id, $invoice_number, $total_amount,
						 $ctv_id = 0,
						 $kind,
						 $shipping_method,
						 $shipping_address,
						 ...
						 $status = 0,$test_id=0)
	{
		...
		foreach($items as $line=>$item)
		{
			...
			$sale_remarks = 'Online SALE '.$sale_id;
			$inv_data = array(
	...
	public function get_the_sale($sale_id)
	{
		$this->db->from('sales');
		$this->db->where('sale_id', $sale_id);
	...
	{
		$payments = array();
		$payments[$this->lang->line('order_cod')] = $this->lang->line('order_cod');
		$payments[$this->lang->line('order_bank')] = $this->lang->line('order_bank');

		if($giftcard)
		{
			$payments[$this->lang->line('order_giftcard')] = $this->lang->line('order_giftcard');
		}
		return $payments;
	}
